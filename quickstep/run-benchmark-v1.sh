...
function run_queries {
  ...
  #  QSPREP="export GLIBC=/fastdisk/glibc-master-build/ GCONV_PATH=${GLIBC}/iconvdata LC_ALL=C" 
  ...
  for query in ${queries[@]} ; do
    ...
    do
      cat /fastdisk/SQL-benchmark/tpch/quickstep/$query.sql >> tmp.sql
    done 
    $QSEXE < tmp.sql >> /fastdisk/SQL-benchmark/tpch/quickstep/test_10_3/${query}_c.out
    rc=$?
    if [ $rc = 124 ] ;
    then
